...
impl FrameBuilder {
    ...
    /// * `hint` - A [`NextFrameHint`] generated by a previous [`FrameBuilder`] or the default hint.
    ...
    pub fn new(
        ...
        let info = FrameInfoBuilder::new(window_id, frame_id, root_id, root_size, hint.info_capacity);
    ...
    pub fn finalize(mut self) -> BuiltFrame {
        self.close_widget_display();

        self.info.set_meta(self.info_id, self.meta);

        let (pipeline_id, display_list) = self.display_list.finalize();
        ...
        let next_frame_hint = NextFrameHint {
            data_capacity: payload.data.len() * 2,
            info_capacity: info.lookup.len() * 2,
        };
...
pub struct BuiltFrame {
    ...
    /// Hint for the next [`FrameBuilder`] created in a similar context.
    pub next_frame_hint: NextFrameHint,
}

/// A hint for the next [`FrameBuilder`] from the previous,
/// contains information that helps optimizing memory allocation.
#[derive(Clone, Copy)]
pub struct NextFrameHint {
    data_capacity: usize,
    info_capacity: usize,
}
impl Default for NextFrameHint {
    fn default() -> Self {
        Self {
            data_capacity: 2.megabytes().bytes(),
            info_capacity: 2000,
        }
    }
}
...
impl FrameUpdate {
    ...
    /// * `hint` - A [`NextFrameUpdateHint`] generated by a previous [`FrameUpdate`] or the default hint.
    pub fn new(
        window_id: WindowId,
        root_id: WidgetId,
        root_transform_key: WidgetTransformKey,
    ...
    pub fn finalize(mut self) -> BuiltFrameUpdate {
        ...
        let next_update_hint = NextFrameUpdateHint {
            scrolls_capacity: self.scrolls.len() * 2,
            transforms_capacity: self.bindings.transforms.len() * 2,
            floats_capacity: self.bindings.floats.len() * 2,
            colors_capacity: self.bindings.colors.len() * 2,
        };
...
pub struct BuiltFrameUpdate {
    ...
    /// Hint for the next [`FrameUpdate`] created in a similar context.
    pub next_update_hint: NextFrameUpdateHint,
}

/// A hint for the next [`FrameUpdate`] from the previous,
/// contains information that helps optimizing memory allocation.
#[derive(Clone, Copy)]
pub struct NextFrameUpdateHint {
    scrolls_capacity: usize,
    transforms_capacity: usize,
    floats_capacity: usize,
    colors_capacity: usize,
}
impl Default for NextFrameUpdateHint {
    fn default() -> Self {
        Self {
            scrolls_capacity: 50,
            transforms_capacity: 500,
            floats_capacity: 500,
            colors_capacity: 500,
        }
    }
