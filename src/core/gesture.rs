...
pub enum ClickArgsSource {
    ...
    /// Click event was generated by a shortcut.
    Shortcut(Shortcut),
}
...
impl AppExtension for GestureManager {
    ...
    fn update(&mut self, request: UpdateRequest, ctx: &mut AppContext) {
        ...
        if self.shortcut_input.has_updates(ctx.events) {
            let config = ctx.services.req::<Gestures>();
            if !config.click_focused.is_empty() {
                for args in self.shortcut_input.updates(ctx.events) {
                    ...
                    if config.click_focused.contains(&args.shortcut) {
                        self.click.notify(
                            ...
                            ClickArgs::new(
                                ...
                                args.window_id,
                                args.device_id,
                                ClickArgsSource::Shortcut(args.shortcut),
