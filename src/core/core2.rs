...
use fnv::FnvHashMap;
use glutin::event::{DeviceEvent, DeviceId, WindowEvent};
...
pub trait UiNode: 'static {
    ...
    /// indicate the parent will acomodate any size.
    ///
    /// # Return
...
bitflags! {
    ...
    pub struct UpdateFlags: u8 {
        ...
        const UPD_HP = 0b000_00010;
        const LAYOUT = 0b000_00100;
        const RENDER = 0b000_01000;
    }
}
...
impl AppRegister {
    ...
    pub fn listener<E: EventNotifier>(&self) -> Option<UpdateNotice<E::Args>> {
        if let Some(any) = self.events.get(&TypeId::of::<E>()) {
            any.downcast_ref::<UpdateNotice<E::Args>>().map(|l| l.clone())
        } else {
            None
        }
    }
...
impl<E: AppExtension> App<E> {
    ...
    pub fn default() -> App<(MouseEvents, KeyboardEvents)> {
        App {
            extensions: (MouseEvents::new(), KeyboardEvents::new()),
        }
    }
