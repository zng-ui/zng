...
pub struct AccessTreeUpdate {
    ...
    // TODO, what if is just property update? (the accesskit crate just replaces the node entry
    // (so we need to have the right count of children!))
    // we need something generated by AccessTree?
    //   - No, this makes removal weird, we just rebuild the WidgetInfoTree branch, lets make the same here.
