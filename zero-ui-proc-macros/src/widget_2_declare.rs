...
use syn::{Attribute, Ident, ItemMod, LitBool, parse::Parse};

use crate::{util::{self, parse_all}, widget_new2::BuiltWhen};
    ...
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        while !input.is_empty() {
            if input.peek(keyword::inherited) {
                inherited.push(util::non_user_braced_id(input,"inherited").parse().unwrap_or_else(|e| non_user_error!(e)))
            } else if input.peek(keyword::widget) {
                let widget = util::non_user_braced_id(input, "widget").parse().unwrap_or_else(|e| non_user_error!(e));
    ...
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        Ok(
            WidgetItem{
                ...
                mixin: named_braces("mixin").parse::<LitBool>().unwrap_or_else(|e| non_user_error!(e)).value,
...
impl Parse for PropertyItem {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {        
        ...
        let property_item = PropertyItem{
            ...
            docs: named_braces("docs").parse().unwrap(),
            cfg: named_braces("cfg").parse().unwrap(),
            default: named_braces("default").parse::<LitBool>().unwrap_or_else(|e| non_user_error!(e)).value,
            required: named_braces("required").parse::<LitBool>().unwrap_or_else(|e| non_user_error!(e)).value,    
        };
