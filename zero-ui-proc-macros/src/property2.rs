...
mod output {
    ...
    pub enum PropertyDocType {
        Type(Box<Type>),
        Impl(Box<Punctuated<TypeParamBound, Token![+]>>),
    }

    impl ToTokens for PropertyDocType {
        fn to_tokens(&self, tokens: &mut TokenStream) {
            match self {
                PropertyDocType::Type(t) => t.to_tokens(tokens),
                PropertyDocType::Impl(b) => tokens.extend(quote!(impl #b)),
            }
        }
    }
    ...
    impl fmt::Display for PropertyDocType {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            let ty_str = match self {
                PropertyDocType::Type(t) => t.to_token_stream().to_string(),
                PropertyDocType::Impl(p) => {
                    write!(f, "<span class='kw'>impl</span> ")?;
