...
use zero_ui_layout::units::{Dip, DipPoint, DipToPx, Factor};
...
event_args! {
    ...
    pub struct MouseClickArgs {
        ...
        /// If the event was generated by holding the button pressed over an widget with [`ClickMode::repeat`].
        pub is_repeat: bool,
...
impl MouseManager {
    fn on_mouse_input(&mut self, window_id: WindowId, device_id: DeviceId, state: ButtonState, button: MouseButton) {
        ...
        let wgt_tree = match WINDOWS.widget_tree(window_id) {
            Ok(t) => t,
            Err(e) => {
