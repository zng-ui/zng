...
mod ansi_fn {
    ...
    context_var! {
        ...
        ///
        /// The returned widgets are layout by the [`LINE_GEN_VAR`]. The default view is [`default_text_fn`].
        pub static TEXT_GEN_VAR: WidgetFn<TextFnArgs> = wgt_fn!(|args: TextFnArgs|  {
            default_text_fn(args)
        });

        /// Widget function for [`LineFnArgs`].
        ///
        /// The returned widgets are layout by the [`PAGE_GEN_VAR`]. The default view is [`default_line_fn`].
        pub static LINE_GEN_VAR: WidgetFn<LineFnArgs> = wgt_fn!(|args: LineFnArgs| {
            default_line_fn(args)
        });

        /// Widget function for [`PageFnArgs`].
        ///
        /// The returned widgets are layout by the [`PANEL_GEN_VAR`] widget. The default view is [`default_page_fn`].
        pub static PAGE_GEN_VAR: WidgetFn<PageFnArgs> = wgt_fn!(|args: PageFnArgs| {
