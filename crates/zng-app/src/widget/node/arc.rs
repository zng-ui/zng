...
mod impls {
    ...
    impl<T: TakeOn> UiNodeImpl for TakeSlot<T> {
        ...
        fn measure_list(
            ...
            self.delegate_owned_mut(|n|n.0.measure_list(wm, measure, fold_size)).unwrap_or_default()
        }
        ...
        fn layout(&mut self, wl: &mut WidgetLayout) -> PxSize {
         ...
         fn layout_list(
            ...
            self.delegate_owned_mut(|n| n.0.layout_list(wl, layout, fold_size)).unwrap_or_default()
        }
        ...
        fn as_widget(&mut self) -> Option<&mut dyn WidgetUiNodeImpl> {
            // check directly to avoid into_widget wrapping widgets
            if self.rc.item.lock().0.as_widget().is_some() {
                Some(self)
            } else {
                None
            }
