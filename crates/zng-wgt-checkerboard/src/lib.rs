...
pub struct Colors(pub [RgbaPair; 2]);
impl ops::Deref for Colors {
    type Target = [RgbaPair; 2];
...
impl_from_and_into_var! {
    fn from<C: Into<RgbaPair>>([c0, c1]: [C; 2]) -> Colors {
        Colors([c0.into(), c1.into()])
    }
    fn from<C0: Into<RgbaPair>, C1: Into<RgbaPair>>((c0, c1): (C0, C1)) -> Colors {
        Colors([c0.into(), c1.into()])
    }
}
...
context_var! {
    ...
    pub static COLORS_VAR: Colors = [
        rgba_pair(rgb(20, 20, 20), rgb(202, 202, 204)),
        rgba_pair(rgb(40, 40, 40), rgb(253, 253, 253)),
    ];
...
pub fn node() -> impl UiNode {
    ...
    match_node_leaf(move |op| match op {
        ...
        UiNodeOp::Render { frame } => {
            ...
            let colors = [c0.rgba(sch), c1.rgba(sch)];
