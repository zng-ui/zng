...
pub fn best(
    ...
    match font_kit::source::SystemSource::new().select_best_match(
        ...
        Err(font_kit::error::SelectionError::CannotAccessSource { reason }) => Err(FontLoadingError::Io(Arc::new(std::io::Error::new(
...
fn workaround_ubuntu(style: FontStyle, weight: FontWeight, stretch: FontStretch) -> Result<Option<(FontDataRef, u32)>, FontLoadingError> {
    ...
    let ubuntu = match source.select_family_by_name("Ubuntu") {
        ...
        Err(e) => {
            return match e {
                ...
                font_kit::error::SelectionError::CannotAccessSource { reason } => Err(FontLoadingError::Io(Arc::new(std::io::Error::new(
                    std::io::ErrorKind::Other,
                    reason.unwrap_or_default(),
                )))),
            }
