...
* Run all tools that requested `zng-res::on-final=`.
...
Tools must implement this CLI:

```console
cargo-zng-res-{tool} [OPTIONS] <SOURCE> <TARGET> <REQUEST>
...
Options:
      ...
      --help
          Print help (uses by `cargo zng res --list`)
...
* `zng-res::on-final={args}` — Subscribe to be called with `{args}` after all tools have run.
