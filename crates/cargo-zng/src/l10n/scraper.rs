...
impl FluentTemplate {
    ...
    pub const AUTO_GENERATED_HEADER: &str = "### Auto generated by `cargo zng l10n`\n\n";
    ...
    pub fn write(&self, mut write_file: impl FnMut(&str, &str) -> io::Result<()> + Send + Sync) -> io::Result<()> {
        let mut file = None;
        let mut output = Self::AUTO_GENERATED_HEADER.to_owned();
        let mut section = "";
        let mut id = "";
        ...
        for (i, entry) in self.entries.iter().enumerate() {
            if file != Some(&entry.file) {
                if let Some(prev) = &file {
                    ...
                    output.clear();
                    output.push_str(Self::AUTO_GENERATED_HEADER);
                    section = "";
                    id = "";
                }
